...
async function obtainNELLock() {
  await fetch(
    "/network-error-logging/support/lock.py?op=lock&reportID=" + reportID);
}

async function releaseNELLock() {
  await fetch(
    "/network-error-logging/support/lock.py?op=unlock&reportID=" + reportID);
}
...
async function clearReportingAndNELConfigurations(subdomain) {
  await fetch(getURLForClearingConfiguration(), {mode: "no-cors"});
  await fetch(getURLForClearingConfiguration("www2"), {mode: "no-cors"});
  return;
}
